...
Merges:
  ...
  - backend/static/live2d/vampire/* → dist-pages/live2d/vampire/  (Live2D model)
...
MODEL_SRC = ROOT / "backend" / "static" / "live2d" / "vampire"
...
def copy_model():
    ...
    for src in MODEL_SRC.rglob("*"):
        if src.is_file():
            ...
            n += 1
    print(
        f"  -> {n} files copied ({sum(f.stat().st_size for f in DEST_MODEL.rglob('*') if f.is_file()) / 1e6:.1f} MB)"
    )
...
def patch_widget_model_path():
    ...
    if count == 0:
        print(
            "WARNING: DEFAULT_MODEL pattern not found in widget.html — patch may be already applied or pattern changed"
        )
    else:
...
/*
  ...
  # 2026-06-13 v9: drop X-Frame-Options + Content-Security-Policy + Permissions-Policy
  # per user request #17185. User confirmed widget worked on Android mobile
  # before any security headers were added (#17127). Hypothesis: even after
  # v8 dropped frame-ancestors, the remaining security headers still trigger
